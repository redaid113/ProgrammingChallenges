...
void initVector(struct vector *v)
{
    v->size = initialSize;
    v->used = 0;
    v->values = (int *)malloc(sizeof(int) * initialSize);
}

void add(struct vector *v, int element)
{
    if (v->used == v->size)
    {
        v->size *= 2;
        v->values = (int *)realloc(v->values, v->size * sizeof(int));
    }
    v->values[v->used++] = element;
}
...
{
    add(&(fsm->values), value);
}

void addValues(struct trie *first)
{
    for (int i = 0; i < first->fail->values.used; i++)
    {
        add(&(first->values), first->fail->values.values[i]);
    }
}
...
unsigned long long getHealth(struct vector values, int *health, int first, int last)
{
    unsigned long long sum = 0;
    for (int i = 0; i < values.used; i++)
    {
        if (first <= values.values[i] && values.values[i] <= last)
        {
            sum += health[values.values[i]];
        }
    }
    return sum;
}
...
unsigned long long int search(struct trie *fsm, char *d, int first, int last, int *health)
{
    ...
    while (d[i] != '\0')
    {
        ...
        {
            cur = found;
            struct trie *temp = cur;
            while (temp != NULL)
            {
                sum += getHealth(temp->values, health, first, last);
                temp = temp->fail;
            }
        }
        i++;
